...
class AdaGAE(torch.nn.Module):
    ...
    def forward(self, Laplacian):
        # sparse
        z = Laplacian.mm(self.X.matmul(self.W1))
        z = Laplacian.mm(z)
        z = Laplacian.mm(z)
        z = torch.nn.functional.relu(z)
        z = Laplacian.mm(z.matmul(self.W2))
        self.embedding = z.matmul(self.W3)
        self.embedding_agg = Laplacian.mm(self.embedding)
        return self.embedding

    def update_graph(self):
        embedding = self.embedding
        self.embedding_agg = None
        torch.cuda.empty_cache()
        weights, raw_weights = utils.cal_weights_via_CAN(embedding.t(), self.num_neighbors, self.links)  # first
        weights = weights.detach()
        raw_weights = raw_weights.detach()
        Laplacian = utils.get_Laplacian_from_weights(weights)
        S_hat = (torch.ones_like(raw_weights) - raw_weights.bool().int()) / (self.X.shape[0] - self.num_neighbors)
        S_hat = S_hat.detach()
        return weights, Laplacian, raw_weights, S_hat

    def build_loss(self, S, S_hat):
        n = self.X.shape[0]
        loss = torch.sum(torch.pow(self.embedding - self.embedding_agg, 2)) / n
        self.base = torch.pow(self.embedding - self.embedding_agg, 2).mean().item()
        weights = (S + S.t()) / 2
        degree = weights.sum(dim=1)
        L = torch.diag(degree) - weights
        loss += self.lam * torch.trace(self.embedding.t().matmul(L).matmul(self.embedding)) / n
        p_hat_Laplacian = utils.get_Laplacian_from_weights(S_hat)
        t = torch.pow(self.embedding - p_hat_Laplacian.mm(self.embedding), 2).sum(dim=1)
        loss += 1 * torch.exp(-1 * t + self.base).mean()
    ...
    def run(self):
        ...
        for epoch in range(self.max_epoch):
            ...
            for i in range(self.max_iter):
                optimizer.zero_grad()
                self(Laplacian)
                loss = self.build_loss(raw_weights, S_hat)
                weights = weights.cpu()
            ...
            if self.num_neighbors < self.max_neighbors:
                ...
                Laplacian = None
                torch.cuda.empty_cache()
                weights, Laplacian, raw_weights, S_hat = self.update_graph()
